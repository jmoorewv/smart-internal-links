...
class Smart_Links_Content_Processor {
    ...
    public function process_text( $text, bool $is_comment ): string {
        ...
        // FIX: Properly handle 0 as unlimited for maxlinks
        $maxlinks = isset( $this->options['maxlinks'] ) ? intval( $this->options['maxlinks'] ) : 3;
        ...
        // CRITICAL FIX: Protect existing links from being modified
        $link_placeholders = [];
        $link_counter = 0;
        $text = preg_replace_callback(
            '/<a\s+[^>]*>.*?<\/a>/is',
            function( $matches ) use ( &$link_placeholders, &$link_counter ) {
                $placeholder = '<!--LINK_PLACEHOLDER_' . $link_counter . '-->';
                $link_placeholders[$link_counter] = $matches[0];
                $link_counter++;
                return $placeholder;
            },
            $text
        ...
        if ( ! empty( $link_placeholders ) ) {
            for ( $i = 0; $i < $link_counter; $i++ ) {
                ...
                $text = str_replace( $placeholder, $link_placeholders[$i], $text );
            }
        }
